...
| `KeyboardUtils.swift` | Shared utilities: `rewriteEvent` (in-place CGEvent modification), `postKey` (CGEvent key synthesis), `isTerminalApp`/`isFinderApp` (bundle ID checks), `isFocusedOnTextField` (Accessibility API role check), `isMissionControlActive` (Dock overlay window count), `toAXElement`/`toAXValue` (CF type helpers), `terminalBundleIDs` set. Used by HomeEndHandler, GnomeShortcutHandler, FinderCutHandler, MenuBarBackground, and MissionControlCloseHandler. |
