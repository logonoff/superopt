...
- **Menu Key → Shortcut Menu** → The Menu/Application key on PC keyboards (key code 0x6E) posts a right-click at the current mouse position, providing consistent shortcut menu access across all apps. Own handler class (`MenuKeyHandler`), own UserDefaults key (`menuKeyRightClickEnabled`). Disabled by default.
...
| `KeyboardUtils.swift` | Shared utilities: `rewriteEvent` (in-place CGEvent modification), `postKey` (CGEvent key synthesis), `isTerminalApp`/`isFinderApp` (bundle ID checks), `isFocusedOnTextField` (Accessibility API role check), `isMissionControlActive` (Dock overlay window count), `pressTilingMenuItem` (AX menu bar search for tiling actions), `toAXElement`/`toAXValue` (CF type helpers), `terminalBundleIDs` set. Used by HomeEndHandler, GnomeShortcutHandler, FinderCutHandler, MenuBarBackground, MissionControlCloseHandler, WindowTilingHandler, and ZoomButtonHandler. |
